...
class ConnectFourBoard(object):
    ...
    def _max_length_from_cell(self, row, col): #has used
    ...
    def _contig_vector_length(self, row, col, direction): #has used
        """
    ...
    def chain_cells(self, playerid):
        ...
        return retVal
### my changes
    def _contig_vector_cells_2(self, row, col, direction):
        """
        Starting in the specified cell and going a step of direction = (row_step, col_step),
        count how many consecutive cells are owned by the same player as the starting cell.
        """
        ...
        while 0 <= row < self.board_height and 0 <= col < self.board_width :
            if(playerid == self.get_cell(row, col)):
                retVal.append((row, col))
                row += direction[0]
                col += direction[1]
            #if(0 == self.get_cell(row, col) and len(retVal)==3

        return retVal[1:]

    def _chain_sets_from_cell_2(self, row, col):
        ###Return the max-length chain containing this cell"""
        ###diagpos
	temp = reverse(self._contig_vector_cells_2(row, col, (1,1))) + [(row, col)] + self._contig_vector_cells(row, col, (-1,-1))
        if(len(temp)==3):#ba toole 3
                if((0<=temp[0][0]-1<self.board_height and 0<=temp[0][0]-1<self.board_width and board(temp[0][0]-1,temp[0][1]-1)==0)):
                    diagpos = temp
                elif((0<=temp[2][0]-1<self.board_height and 0<=temp[2][0]-1<self.board_width and board(temp[2][0]+1,temp[2][1]+1)==0)):
                    diagpos = temp
                else:
                    diagpos = []
        if(len(temp)==2):
                if((0<=temp[0][0]-1<self.board_height and 0<=temp[0][0]-1<self.board_width and board(temp[0][0]-1,temp[0][1]-1)==0)):
                    diagpos = temp
                elif((0<=temp[1][0]-1<self.board_height and 0<=temp[1][0]-1<self.board_width and board(temp[1][0]+1,temp[1][1]+1)==0)):
                    diagpos = temp
                else:
                    diagpos = []
        else :
        ...
        if(len(temp)==3):
                if((0<=temp[0][0]-1<self.board_height and 0<=temp[0][0]+1<self.board_width and board(temp[0][0]-1,temp[0][1]+1)==0)):
                    diagneg = temp
                elif((0<=temp[0][2]+1<self.board_height and 0<=temp[2][0]-1<self.board_width and board(temp[2][0]+1,temp[2][1]-1)==0)):
                    diagneg = temp
                else:
                    diagneg = []
        if(len(temp)==2):
                if((0<=temp[0][0]-1<self.board_height and 0<=temp[0][0]+1<self.board_width and board(temp[0][0]-1,temp[0][1]+1)==0)):
                    diagneg = temp
                elif((0<=temp[0][1]+1<self.board_height and 0<=temp[1][0]-1<self.board_width and board(temp[1][0]+1,temp[1][1]-1)==0)):
                    diagneg = temp
                else:
                    diagneg = []
        else :
            diageg = temp
        ###vertical
        temp = reverse(self._contig_vector_cells_2(row, col, (1,0))) + [(row, col)] + self._contig_vector_cells(row, col, (-1,0))
        if(len(temp)==3):
                if((0<=temp[0][0]+1<self.board_height and 0<=temp[0][0]+0<self.board_width and board(temp[0][0]+1,temp[0][1]+0)==0)):
                    vertical = temp
                elif((0<=temp[2][0]-1<self.board_height and 0<=temp[2][0]+0<self.board_width and board(temp[2][0]-1,temp[2][1]+0)==0)):
                    vertical = temp
                else:
                    vertical = []
        if(len(temp)==2):
                if((0<=temp[0][0]+1<self.board_height and 0<=temp[0][0]+0<self.board_width and board(temp[0][0]+1,temp[0][1]+0)==0)):
                    vertical = temp
                elif((0<=temp[1][0]-1<self.board_height and 0<=temp[1][0]+0<self.board_width and board(temp[1][0]-1,temp[1][1]+0)==0)):
                    vertical = temp
                else:
                    vertical = []
        else :
            vertical = temp
        ###horizental
        temp = reverse(self._contig_vector_cells_2(row, col, (0,1))) + [(row, col)] + self._contig_vector_cells(row, col, (0,-1))
        if(len(temp)==3):
                if((0<=temp[0][0]+0<self.board_height and 0<=temp[0][0]+1<self.board_width and board(temp[0][0]+0,temp[0][1]+1)==0)):
                    horizental = temp
                elif((0<=temp[2][0]+0<self.board_height and 0<=temp[2][0]-1<self.board_width and board(temp[2][0]+0,temp[2][1]-1)==0)):
                    horizental = temp
                else:
                    horizental = []
        elif(len(temp)==2):
                if((0<=temp[0][0]+0<self.board_height and 0<=temp[0][0]+1<self.board_width and board(temp[0][0]+0,temp[0][1]+1)==0)):
                    horizental = temp
                elif((0<=temp[1][0]+0<self.board_height and 0<=temp[1][0]-1<self.board_width and board(temp[1][0]+0,temp[1][1]-1)==0)):
                    horizental = temp
                else:
                    horizental = []
        else :
	...
	return [ tuple(x) for x in [diagpos, vertical, horizental, diagneg ] ]

    
### not to count useless chains  
    def usefull_chain_cells(self, playerid):

        retVal = set()
        for i in xrange(self.board_height):
            for j in xrange(self.board_width):
                if self.get_cell(i,j) == playerid:
                    retVal.update( self._chain_sets_from_cell_2(i,j) )
